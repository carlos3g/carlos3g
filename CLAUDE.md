...
- **Releasing:** automated via `.github/workflows/release.yml`. On every push to `master` that touches `bin/**` or `package.json`, the workflow checks npm for the current `package.json#version`; if that version isn't published, it runs `npm publish` and cuts a `vX.Y.Z` GitHub release. To ship: bump `version`, commit, push — the workflow handles the rest.
- **Auth model:** the workflow uses npm **Trusted Publishing** (OIDC) — no `NPM_TOKEN` secret. The trust is configured once at https://www.npmjs.com/package/carlos3g/access (Publishing access → Trusted Publisher) tied to this repo + `release.yml`. Trusted publishing requires npm ≥ 11.5.1 and Node ≥ 22.14, hence `node-version: '24'` in the workflow. Provenance attestations are generated automatically.
